...
class NodeNetworkNavigation(BaseNavigation):
    ...
    def reset(self, vehicle, dest=None, random_seed=None):
        ...
        self.generate_dense_route_points(current_lane.index[-1], spacing=10.0)
    ...
    def generate_dense_route_points(self, road_index: int, spacing: float = 2.0):
        self.route_points = []
        self.right_lane = []
        # lane_width = self.get_current_lane_width()
        # later_middle = (float(self.get_current_lane_num()) / 2 - 0.5) * lane_width

        for ckpt1, ckpt2 in zip(self.checkpoints[:-1], self.checkpoints[1:]):
            ref_lane = self.map.road_network.graph[ckpt1][ckpt2][road_index]
            length = max(ref_lane.length, 0.0001)
            n = max(1, int(np.ceil(length / spacing)))
            
            for i in range(n):
                long_pos = min(i * spacing, length)
                pos = ref_lane.position(long_pos, 0.0)
                pos_array = np.array(pos)
                self.route_points.append(pos_array)
            self.right_lane.append(ref_lane)
