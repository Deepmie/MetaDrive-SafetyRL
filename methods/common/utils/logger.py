from methods.common.base_config import LoggerConfig
...
class Logger:
    def __init__(self, method_name: str, config: LoggerConfig):
        self.config = config
        self.now_datetime = datetime.now()
        
        # =================日志路径名================= #
        logger_path: str           = os.path.join(config.logger_path_root, '{}/log_{}'.format(method_name, self.now_datetime.strftime("%Y_%m_%d_%H_%M_%S")))
    ...
    def _get_config_modules_dynamic(self) -> Dict[str, Dict]:
        ...
        for key, value in CONFIG_DICT.items():
            ...
            module_total_name = f'methods.common.{main_module}'
