...
from methods.common.policy import Policy
from methods.common.buffer import RolloutBuffer, RolloutBatchData
from methods.common.envs import ParallelEnv, SingleEnv
from methods.common.schedule import ConstantSchedule
from methods.common.utils import converto_ndarray, converto_torch, Logger, Timer
from methods.common.create_env import create_env, create_render_config
from methods.common.type import ActionType, RenderClass
from methods.common.monitor import Monitor
from methods.common.base_alg import BaseAlg
...
class DefaultAlg(BaseAlg):
    def __init__(self, config: BaseAlgConfig, eval_mode: bool = False):
        ...
        self._create_logger(config.method_name)
        ...
        self.monitor: Monitor              = Monitor(self.config.collect_data_path, self.config.method_name, self.eval_mode)
    ...
    def eval_with_checkpoint(self, ckpt_path: str) -> Tuple[float, List[Dict], Dict]:
    ...
    def _evaluate(self, is_render: bool = False, evaluate_save_path: str = '') -> Tuple[float, List[Dict]]:
        ...
        # 创建一个新的控制器
        controller_eval: DefaultController = self._create_controller(env=env_eval, eval_mode=True)
    ...
    def _create_controller(self, env: Union[SingleEnv, ParallelEnv], eval_mode: bool) -> DefaultController:
        return DefaultController(env, config = self.config.controller_config,
            mpc_cls = DefaultMPC, cbf_cls = DefaultCBF, eval_mode = eval_mode)
    
    def _create_logger(self, method_name: str):
        record_path: str = self.config.logger_config.record_path
        if not self.eval_mode:
            self.logger: Logger   = Logger(method_name, self.config.logger_config)
            ...
            record_tmp_path: str = record_path + '.tmp'
            if not os.path.exists(record_path):
                with open(record_path, mode='w', encoding='utf-8') as writer:
                    writer.write('{}')
            record_file = open(record_path, mode='r', encoding='utf-8')
            record_tmp_file = open(record_tmp_path, mode='w', encoding='utf-8')
            # operate start
            content: Dict = json.loads(record_file.read())
            content[self.config.method_name] = self.logger_path
            record_tmp_file.write(json.dumps(content, ensure_ascii=False, indent=2))
            # operate end
            record_file.close(); record_tmp_file.close()
            ...
            self.logger_path: str = content.get(self.config.method_name)
