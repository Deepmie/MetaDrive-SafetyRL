...
@dataclass
class LoggerConfig:
    logger_path_root: str = 'deep/logger'
    record_path: str      = 'deep/logger_path.json'
...
class BaseAlgConfig:
    ...
    def _set_special_attr(self, cls, attr_name: str, attr_value: Union[int, float, str]):
        if id(cls) in self._map: return # 哈希表确保只递归一次
        self._map.add(id(cls))
        
        if self._hasattr(cls, attr_name):
            self._setattr(cls, attr_name, attr_value)
        
        iters = list(cls.keys()) if isinstance(cls, dict) else [f.name for f in fields(cls)]
        for name in iters:
            attr = self._getattr(cls, name)
            if is_dataclass(attr) or isinstance(attr, dict):
                self._set_special_attr(attr, attr_name, attr_value)
    
    def _hasattr(self, cls, attr_name: str):
        if isinstance(cls, dict): return attr_name in cls
        else: return hasattr(cls, attr_name)
    
    def _getattr(self, cls, attr_name: str):
        if isinstance(cls, dict): return cls.get(attr_name)
        else: return getattr(cls, attr_name)
    
    def _setattr(self, cls, attr_name: str, attr_value: Union[int, float, str]):
        if isinstance(cls, dict): cls[attr_name] = attr_value
        else: setattr(cls, attr_name, attr_value)
